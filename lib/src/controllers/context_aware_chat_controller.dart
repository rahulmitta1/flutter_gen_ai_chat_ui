...
class ContextAwareChatController extends ChangeNotifier {
  ...
  Future<void> sendMessage(
    ChatMessage message, {
    required Future<String> Function(String enhancedPrompt, List<AiAction> availableActions) onAiResponse,
  }) async {
    ...
    try {
      ...
      final aiResponseText = await onAiResponse(enhancedPrompt, availableActions);
  ...
  String _createEnhancedPrompt(String originalMessage) {
    ...
    buffer.writeln('\\n--- Instructions ---');
    buffer.writeln('Use the provided context to give more relevant and helpful responses.');
  ...
  Stream<String> sendMessageStream(
    ChatMessage message, {
    required Stream<String> Function(String enhancedPrompt, List<AiAction> availableActions) onAiResponseStream,
  }) async* {
    // Add user message first
    _chatController.addMessage(message);
    ...
    try {
      // Stream AI response
      await for (final chunk in onAiResponseStream(enhancedPrompt, availableActions)) {
        buffer.write(chunk);
  ...
  Future<ActionResult> executeAction(String actionName, Map<String, dynamic> parameters) {
  ...
  void _onContextChanged() {
    ...
    debugPrint('Context updated: ${_readableController.contextKeys.length} items');
  }
  
  void _onActionsChanged() {
    // Actions have changed
    debugPrint('Actions updated: ${_actionController.registeredActions.length} actions');
  }
...
class ContextAwareChatProvider extends StatefulWidget {
  ...
  @override
  State<ContextAwareChatProvider> createState() => _ContextAwareChatProviderState();
}
...
mixin ContextAwareChatMixin<T extends StatefulWidget> on State<T> {
  ...
  ContextAwareChatController get contextAwareChat {
    return _contextAwareChatController ??= context.contextAwareChat ?? ContextAwareChatController();
  }
